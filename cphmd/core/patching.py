...
class PatchParser:
    ...
    def register_ligand(self, ligand_def: LigandPatchDef) -> None:
        ...
        The reference patch (resname + "O", e.g., NREDO for NRED) is automatically
        parsed to ensure all atoms including appearing/disappearing protons are
        included in the REPLICATE selection.
        ...
        # Reference patch name (auto-generated by block_builder as resname + "O")
        ref_patch_name = resname + "O"

        # Parse ALL patches including reference to get complete atom list
        all_patch_names = [ref_patch_name] + list(ligand_def.patches)
        patch_atoms, patch_pka = self._parse_ligand_patch_file(
            patch_file, all_patch_names
        )
        ...
        # Collect all atoms across ALL patches (including reference) for REPLICATE
        all_atoms: set[str] = set()

        # Include reference patch atoms (for appearing/disappearing atoms like H36)
        ref_patch_upper = ref_patch_name.upper()
        if ref_patch_upper in patch_atoms:
            self.atom_groups[ref_patch_upper] = patch_atoms[ref_patch_upper]
            all_atoms.update(patch_atoms[ref_patch_upper])

        # Include alternate patch atoms
        ...
        # Set all atoms for the residue (union of reference + alternates)
        self.atom_groups[resname] = list(all_atoms)

        print(f"Registered ligand {resname} with patches: {self.patches[resname]}")
        print(f"  Reference patch {ref_patch_upper}: {self.atom_groups.get(ref_patch_upper, [])}")
