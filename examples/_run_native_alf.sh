...
RANK_WRAPPER="$SCRIPT_DIR/_run_native_rank.sh"
...
for env_name in CUDA_VISIBLE_DEVICES SLURM_JOB_GPUS SLURM_STEP_GPUS; do
    if [[ -n "${!env_name:-}" ]]; then
        mpi_env_args+=(-x "$env_name")
    fi
done

if [[ ! -f state/initialized.json ]]; then
...
mpirun -np "$NREPS" \
    ...
    "${mpi_env_args[@]}" \
    "$RANK_WRAPPER" "$python_cmd" -m cphmd.cli.main run -c "$CONFIG_PATH"
