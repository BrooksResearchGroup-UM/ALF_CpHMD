...
def parse_additional_arguments():
    # Create the parser
    parser = argparse.ArgumentParser(description='Process additional parameters for the simulation.')

    # Define the expected command line arguments
    parser.add_argument('--hmr', type=bool, default=False, help='Whether to use hydrogen mass repartitioning (True/False)')
    parser.add_argument('-i', '--input_folder', type=str, default='asp', help='Name of the input folder')
    parser.add_argument('--structure_file', type=str, default='solvated', help='Name of the structure file')

    # Parse the arguments
    args = parser.parse_args()

    # Assign global variables
    global hmr, input_folder, structure_file
    hmr = args.hmr
...
class universe():
    ...
    def update_universe(self):
        ...
        universe = pd.DataFrame({'index': select_all._atom_indexes, 'atom_type': select_all._atom_types, 'res_name': select_all._res_names, 'res_id': select_all._res_ids,
...
parser.add_argument('-i', '--input', dest='input_folder',
                    default=input_folder, help='Input folder')
...
f.write('SEGID,RESID,PATCH,SELECT,TAG\n')
f.close()    
    ...
    # Delete atoms in original residue
        ...
        # Rename replica's resname to patch name
            ...
            if patch != resname+'O':
                ...
                f.write(f'{seg_id},{resid},{patch},s{i}s{j},NONE\n')
