...
class ALF_simulation:
    ...
    def run(self):
        ...
        for i in range(run, self.end+1):
            if self.phase != 3:
                break
            start_time = time.time()
            for j in range(phase3_runs):
    ...
    def crystal(self, run, letter=''):
        ...
        if restart_run == 1:
            ...
            pycharmm.charmm_script('CRYSTAL FREE')
            if os.path.isfile(f'{input_folder}/run{restart_run}/prod.crd{letter}'):
                read.coor_card(f'{input_folder}/run{restart_run}/prod.crd{letter}')
            else:
                read.coor_card(f'{input_folder}/run{restart_run}/prod.crd')
            if os.path.isfile(f'{input_folder}/run{restart_run}/box.dat{letter}'):
                file = open(f'{input_folder}/run{restart_run}/box.dat{letter}').readlines()
            else:
    ...
    def block(self, i):
        ...
        block_command += 'END'
        with open(f'{input_folder}/run{i}/block{letter}.inp', 'w') as f:
    ...
    def scat(self, hydrogen = False):
        ...
        scat_command += 'END'
        with open(f'{input_folder}/run{i}/scat{letter}.inp', 'w') as f:
    ...
    def dynamics(self, run, letter=''):
        ...
        dyn_param = {'start': True, 'restart': False,
                     'blade': True,'cpt': cpt_on, 'leap': True,
                    'langevin': True, 'timestep': 0.002,
                    'firstt': temperature, 'finalt': temperature,
                    'tstruc': temperature, 'tbath': temperature,
                    'iasors': 1, 'iasvel': 1, 'iscvel': 1, 'iscale': 0,
                    'ihtfrq': 0, 'ieqfrq': 0, 'ichecw': 0,
                    'imgfrq': -1, 'ilbfrq': 0,
                    'echeck': -1, 'iunldm': self.lmd_unit,
                    'iunwri': self.rst_unit,'iuncrd': self.dcd_unit
                    }
        #'ihbfrq': 0
        #'inbfrq': -1
        ...
        if nsteps_eq > 0:
            ...
            lmd.close()
            if not cent_ncres:
                dcd = pycharmm.CharmmFile(f'{input_folder}/run{run}/dcd/{name}_eq.dcd', 
                                          file_unit=self.dcd_unit,read_only=True, formatted=False)
                dcd_new = pycharmm.CharmmFile(f'{input_folder}/run{run}/dcd/{name}_eq_1.dcd',
                ...
                os.remove(f'{input_folder}/run{run}/dcd/{name}_eq.dcd')
                os.rename(f'{input_folder}/run{run}/dcd/{name}_eq_1.dcd', f'{input_folder}/run{run}/dcd/{name}_eq.dcd')
                dcd.close()
                dcd_new.close()
        ...
        if nsteps_prod > 0:
            ...
            if sim_type == 'flat':
                rpr_fn = f'{input_folder}/run{run}/res/{name}_eq.rst'
            elif os.path.isfile(f'{input_folder}/run{run-1}/res/{name}_prod.rst{letter}'):
                rpr_fn = f'{input_folder}/run{run-1}/res/{name}_prod.rst{letter}'
            else:
                rpr_fn = f'{input_folder}/run{run-1}/res/{name}_flat.rst'
    ...
    def alf_analysis(self,run):
        try:
            ...
            if self.phase == 3:
                self.alf_info['nreps'] = phase3_runs
                os.makedirs('data', exist_ok=True)
                for j in range(0,phase3_runs):
                    letter = '_' + self.alphabet[j]
                ...
                l_files = os.listdir('../')
                l_files = [file for file in l_files if file.startswith('analysis')]
                l_files = [file for file in l_files if os.path.exists(f'../{file}/data/Lambda.0.1.dat')]
                if len(l_files) < 5:
                    N = len(l_files)
            else:
                self.alf_info.update({'nreps': 1})
            ...
            if np.all(np.abs(np.subtract.outer(col_count, col_count)) < 0.1) and np.all(col_count > 0.1) and self.phase == 1:
                self.phase = 2
                print(f'Run {run}: Moving to phase 2.')
            if np.all(np.abs(np.subtract.outer(col_count, col_count)) < 0.03) and np.all(col_count > 0.1) and self.phase == 2:
                self.phase = 3
            ...
            if alf.RunWham.__code__.co_argcount == 4 and self.fix_bias == False:
                alf.RunWham(N*self.alf_info['nreps'], self.alf_info['temp'], 0, 0)
            ...
            if self.phase == 1 and run < 10:
                alf.GetFreeEnergy5(alf_info=self.alf_info, ms=0,msprof=0, cutb=5)
            elif self.phase == 1 and run >= 10 and run < 20:
                alf.GetFreeEnergy5(self.alf_info, ms=0,msprof=0, cutb=3)
            elif self.phase == 1:
                alf.GetFreeEnergy5(self.alf_info, ms=0,msprof=0, cutb=2)
            if self.phase == 2:
